...
setwd('~/Desktop/GitHub/Data_cleanup')
getwd()
#upload feature table and metadata
md <- read.csv('gnps_metadata_for_mouse_studies_01112023.csv', header = T, check.names = F)
ft <- read.csv('01102023_MZmine_trial_sirius_iimn_gnps_quant_filtered.csv', header = T, check.names = F, sep = ',')
...
write.csv(new_md, "~/Desktop/GitHub/Data_cleanup/md_new.csv",row.names = TRUE)
...
Condition <- as.double(unlist(readline("Enter the index number of the attribute to split sample and blank:"))) #1
Levels_Cdtn <- levels(droplevels(as.factor(data[,Condition[1]])))

#Among the shown levels of an attribute, select the ones to keep
Blk_id <- as.double(unlist(readline("Enter the index number of your BLANK:"))) #2
...
write.csv(Imp_clrt, "CLR_Scaled.csv",row.names = TRUE)
